...
class _MonitorDashboardState extends State<MonitorDashboard> {
  ...
  Future<void> _connect() async {
    try {
      ...
      _pollingTimer = Timer.periodic(const Duration(milliseconds: 120), (timer) {
        ...
        else { _sendRequest("D0${11 + (pollStep - 2)}"); }
        pollStep = (pollStep + 1) % 8;
      });
  ...
  void _disconnect() { _pollingTimer?.cancel(); _socket?.destroy(); _socket = null; setState(() { isConnected = false; statusText = "DISCONNECTED"; }); }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Row(children: [
          ...
          Text(statusText, style: TextStyle(fontSize: 14, color: isConnected ? Colors.blue : Colors.red)),
        ]),
      ...
      body: Column(children: [
        ...
        Padding(
          ...
          child: ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(backgroundColor: isConnected ? Colors.red : const Color(0xFF00539F), minimumSize: const Size(double.infinity, 60)),
            child: Text(isConnected ? "STOP" : "CONNECT ENET"),
          ),
        )
      ]),
    );
  ...
  Widget _buildTile(DisplayParam p, double val, double peak) {
    if (p.id == 'timing_all') return HorizontalTimingChart(corrections: cylinderCorrections, min: p.min);
...
class HorizontalTimingChart extends StatelessWidget {
  ...
  final double min;
  const HorizontalTimingChart({super.key, required this.corrections, required this.min});
  @override
  Widget build(BuildContext context) {
